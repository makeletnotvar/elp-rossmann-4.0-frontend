...
describe('Buildings visibility by permissions as user', () => {
	...
	it('should update USER user permissions', () => {
		cy.visit('/login');
		cy.get('[data-testid="login-username"] input').clear().type(loginCredentials.username);
		cy.get('[data-testid="login-password"] input').clear().type(loginCredentials.password);
		cy.get('[data-testid="login-button"]').click();
		...
		cy.get('[data-testid="edit-user-button"]').should('exist').scrollIntoView();
		cy.get('[data-testid="edit-user-button"]').click({ force: true });
		cy.get('[data-testid="edit-user-permissions-button"]').click();
		cy.get('[data-testid="add-user-permissions-button"]').click();
		cy.get('[data-testid="open-buildings-list-dialog"]').click();
		cy.get('[data-testid="options-select"]').click();
		cy.get(`[data-testid="menu-item-[1337] City"]`).should('exist').scrollIntoView();
		cy.get(`[data-testid="menu-item-[1337] City"]`).click();
		cy.get('[data-testid="dialog-confirm"]').click();
		cy.get('[data-testid="options-select-permissions"]').click();
		cy.get(`[data-testid="menu-item-1"]`).should('exist').scrollIntoView();
		cy.get(`[data-testid="menu-item-1"]`).click();
		cy.get('[data-testid="save-user-permissions-button"]').click();
	});
	...
	it('should be equal by permissions', () => {
		cy.visit('/login');
		...
		cy.get('[data-testid="login-button"]').click();
		cy.wait(1000);
		cy.visit('/buildings/list?o=0&s=1000');
		cy.intercept('GET', '/api/buildings/?o=0&s=1000').as('getBuildings');
		cy.wait('@getBuildings').then(interception => {
			expect(interception.response.statusCode).to.eq(200);
			const buildingsCountByPermissionsAsUser = (interception.response.body.buildings || []).length;
			expect(buildingsCountByPermissionsAsUser).to.equal(1);
		});
		visitPage = '/buildings';
	});
