...
describe('Buildings management operations', () => {
	let visitPage = '/users';
	...
	it('DEV should be able to access the add building', () => {
		cy.visit('/login');
		cy.get('[data-testid="login-username"] input').clear().type(loginCredentials.username);
		cy.get('[data-testid="login-password"] input').clear().type(loginCredentials.password);
		cy.get('[data-testid="login-button"]').click();
		cy.wait(1000);
		cy.visit(visitPage);
		cy.get('[data-testid="buildings-add-button"]').should('be.visible');
		cy.get('[data-testid="buildings-add-button"]').click();
	});

	it('ADMIN should not be able to access the add building', () => {
		cy.visit('/login');
		...
		cy.wait(1000);
		cy.visit(visitPage);
		cy.get('[data-testid="buildings-add-button"]').should('not.exist');
	});

	it('USER should not be able to access the add building', () => {
		cy.visit('/login');
