...
describe('Buildings visibility without permissions and with installation company as user', () => {
	let visitPage = '/users';
	...
	it('should be equal 0 by installation company and not ready for all users', () => {
		cy.visit('/login');
		...
		cy.wait('@getBuildings').then(interception => {
			expect(interception.response.statusCode).to.eq(200);
			const buildingsCountByInstallationCompanyWithoutReadyForAllUsersAsUser = (interception.response.body.buildings || []).filter(
				building => building?.installationCompany?.name.toLocaleLowerCase() === 'el-piast' && building?.readyForAllUsers === false
			).length;
			expect(buildingsCountByInstallationCompanyWithoutReadyForAllUsersAsUser).to.equal(0);
		});
	});
