...
describe('Buildings visibility by installaiton company as user', () => {
	let visitPage = '/users';
	...
	let buildingsCountByInstallationCompany = 0;

	it('should get all buildings count by installation company', () => {
		cy.visit('/login');
		...
		cy.wait(1000);
		cy.visit('/buildings/list?F_installation_company=d6c640e0-8f97-4ab4-a4fc-ce8e64420c38&o=0&s=1000');
		cy.intercept('GET', '/api/buildings/?F_installation_company=d6c640e0-8f97-4ab4-a4fc-ce8e64420c38&o=0&s=1000').as('getBuildings');
		cy.wait('@getBuildings').then(interception => {
			expect(interception.response.statusCode).to.eq(200);
			buildingsCountByInstallationCompany = (interception.response.body.buildings || []).filter(
				building => building?.installationCompany?.name.toLocaleLowerCase() === 'el-piast'
			).length;
		});
	});
	...
	it('should be equal by installation company', () => {
		cy.visit('/login');
		...
		cy.wait('@getBuildings').then(interception => {
			expect(interception.response.statusCode).to.eq(200);
			const buildingsCountByInstallationCompanyAsUser = (interception.response.body.buildings || []).filter(
				building => building?.installationCompany?.name.toLocaleLowerCase() === 'el-piast'
			).length;
			expect(buildingsCountByInstallationCompanyAsUser).to.equal(buildingsCountByInstallationCompany);
		});
	});
