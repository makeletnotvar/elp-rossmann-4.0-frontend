...
describe('Buildings visibility with installation company and service company as user', () => {
	...
	it('should be equal 0 by installation company and service company', () => {
		cy.visit('/login');
		...
		cy.wait('@getBuildings').then(interception => {
			expect(interception.response.statusCode).to.eq(200);
			const buildingsCountByInstallationCompanyAndServiceCompany = (interception.response.body.buildings || []).filter(
				building => building?.installationCompany?.name.toLocaleLowerCase() === 'el-piast' || building?.serviceCompany?.name.toLocaleLowerCase() === 'el-piast'
			).length;
			expect(buildingsCountByInstallationCompanyAndServiceCompany).to.equal(0);
		});
		visitPage = '/companies';
	});
