...
describe('Buildings visibility by permissions and service company as user', () => {
	...
	it('should be equal by permissions and service company', () => {
		cy.visit('/login');
		...
		cy.wait('@getBuildings').then(interception => {
			expect(interception.response.statusCode).to.eq(200);
			const buildingsCountByServiceCompanyAsUser = (interception.response.body.buildings || []).length;
			expect(buildingsCountByServiceCompanyAsUser).to.equal(buildingsCountByServiceCompany + 1);
		});
		visitPage = '/buildings';
	});
