...
describe('Update as admin user admin', () => {
	...
	it('should create a ADMIN2 user', () => {
		cy.visit('/login');
		cy.get('[data-testid="login-username"] input').clear().type(loginCredentials.username);
		cy.get('[data-testid="login-password"] input').clear().type(loginCredentials.password);
		cy.get('[data-testid="login-button"]').click();
	...
	it('should update ADMIN2 user password', () => {
		cy.visit('/login');
		cy.get('[data-testid="login-username"] input').clear().type(loginCredentials.username);
		cy.get('[data-testid="login-password"] input').clear().type(loginCredentials.password);
		cy.get('[data-testid="login-button"]').click();
	...
	it('should update ADMIN2 as ADMIN', () => {
		cy.visit('/login');
		...
		cy.intercept('PUT', `/api/users/${createdUser.uuid}`).as('updateUser');
