...
describe('Login operations', () => {
	...
	it('should log in with correct ADMIN credentials and navigate to the correct page', () => {
		cy.visit('/login');
	...
	it('should fail to log in with incorrect ADMIN password and return 401', () => {
		cy.visit('/login');
		cy.get('[data-testid="login-username"] input').clear().type(adminCredentials.username);
		cy.get('[data-testid="login-password"] input').clear().type('wrongpassword');
		cy.get('[data-testid="login-button"]').click();
		cy.wait(1000);
		cy.get('[data-testid="login-error"]').should('exist').and('be.visible');
	});

	it('should log in with correct USER credentials and navigate to the correct page', () => {
		cy.visit('/login');
	...
	it('should fail to log in with incorrect USER password and return 401', () => {
		cy.visit('/login');
		cy.get('[data-testid="login-username"] input').clear().type(userCredentials.username);
		cy.get('[data-testid="login-password"] input').clear().type('wrongpassword');
		cy.get('[data-testid="login-button"]').click();
