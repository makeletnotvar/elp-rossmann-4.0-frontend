...
describe('Update selft password as user', () => {
	let visitPage = '/users';
	...
	it('should update self password as user', () => {
		cy.visit('/login');
		...
		cy.visit(visitPage);
		cy.get('[data-testid="edit-user-password-button"]').click();
		cy.intercept('PUT', `/api/user/password`).as('updateSelfPassword');
		cy.get('[data-testid="user-current-password"]').type(userCredentials.password);
		cy.get('[data-testid="user-password"]').type('R27.T%RTIdiK');
		cy.get('[data-testid="user-repeat-password"]').type('R27.T%RTIdiK');
		cy.get('[data-testid="save-user-password-button"]').click();
		cy.wait('@updateSelfPassword').then(interception => {
