...
describe('Modules operations', () => {
	...
	it('sould log in as DEV and check module visibility', () => {
		cy.visit('/login');
		cy.get('[data-testid="login-username"] input').clear().type(loginCredentials.username);
		cy.get('[data-testid="login-password"] input').clear().type(loginCredentials.password);
		cy.get('[data-testid="login-button"]').click();
		...
		cy.get('[data-testid="module-dev"]').should('be.visible');
	});

	it('should log in as ADMIN and check if there are any dev modules visible', () => {
		cy.visit('/login');
		...
		cy.wait(1000);
		cy.url().should('include', visitPage);
		cy.get('[data-testid="module-dev"]').should('not.exist');
	});

	it('should log in as USER and check if there are any dev modules visible', () => {
		cy.visit('/login');
