...
describe('Update account type as user', () => {
	let visitPage = '/users';
	...
	it('should not access to the update buildings all', () => {
		cy.visit('/login');
		...
		cy.get('[data-testid="edit-user-button"]').should('exist').scrollIntoView().and('be.visible');
		cy.get('[data-testid="edit-user-button"]').click({ force: true });
		cy.get('[data-testid="user-userBuildingsAll-checkbox"]').should('not.exist');
	});
