...
describe('Devices operations', () => {
	...
	it('DEV should be able to access the devices page and see the add button', () => {
		cy.visit('/login');
		cy.get('[data-testid="login-username"] input').clear().type('dev');
		cy.get('[data-testid="login-password"] input').clear().type('dev');
		cy.get('[data-testid="login-button"]').click();
		cy.wait(1000);
		cy.url().should('include', visitPage);
		cy.visit(devicesPage);
		cy.get('[data-testid="devices-add-button"]').should('be.visible');
		cy.get('[data-testid="devices-add-button"]').click();
	});

	it('ADMIN should not be able to access the devices page and should be redirected to buildings', () => {
		cy.visit('/login');
	...
	it('USER should not be able to access the devices page and should be redirected to buildings', () => {
		cy.visit('/login');
