...
describe('Scenario 3 (DEV) Operations', () => {
	...
	it('should create a new user', () => {
		...
		cy.get('[data-testid="add-company-button"]').click();
		cy.get('[data-testid="options-select"]').click();
		cy.get(`[data-testid="menu-item-${newUser.company}"]`).scrollIntoView().should('be.visible');
		cy.get(`[data-testid="menu-item-${newUser.company}"]`).click();
		cy.get('[data-testid="dialog-confirm"]').click();
	...
	it('should see building in buildings list', () => {
		cy.get('[data-testid^="building-more-button-"]').should('have.length', 1);
		cy.get(`[data-testid="building-more-button-${createdBuilding.uuid}"]`).click({ force: true });
		loginCredentials = {
			...devCredentials,
		};
