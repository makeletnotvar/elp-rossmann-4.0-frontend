...
const TimeItem: React.FC<TimeItemProps> = ({ element, index, isLoading, setIsLoading, selectedDeviceRef, pathStrings }) => {
	...
	const onClickChangeTextInput = useCallback(() => {
		...
		(window as any).vHmi.deviceSendTime({
			value,
			index,
			type: element.type,
		});
	...
	return (
		<VirtualHMIItemView name={element.name} icon={<ScheduleOutlined className={styles.vhmiIcon} />}>
