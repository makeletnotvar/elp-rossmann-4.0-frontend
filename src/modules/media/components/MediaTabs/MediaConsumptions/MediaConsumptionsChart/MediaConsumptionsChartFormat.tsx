...
export const xAxisFormat =
	(settings: MediaConsumptionsRequestSettings, data: ConsumptionData, full: boolean = false) =>
	(indexLabel: any) => {
		...
		switch (settings.period) {
			case HOUR: {
				if (currentDataItem) {
					...
					return moment(startTs).format('hh:mm YYYY-MM-DD');
				} else {
					return '--';
				}
			}
...
export const tooltipFormat =
	(settings: MediaConsumptionsRequestSettings) => (value: string | number | Array<string | number>, name: string, entry: any, index: number) => {
...
export const tooltipLabelFormat =
	(data: ConsumptionData, settings: MediaConsumptionsRequestSettings): any =>
	(indexLabel: any) => {
