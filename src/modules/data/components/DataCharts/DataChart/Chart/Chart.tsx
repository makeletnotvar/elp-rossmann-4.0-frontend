...
import ChartLegend from 'modules/data/components/DataCharts/DataChart/Chart/ChartLegend';
...
interface PointChartProps extends Pick<DataChartProps, 'data' | 'points' | 'from' | 'to' | 'index'> {
	hideXLabels?: boolean;
 ...
 *
 */
const Chart: React.FC<PointChartProps> = ({ points, data, hideXLabels = false, isLastChartContainer, from, to }) => {
	const { yAxises, pointsOfYAxises } = getMergedYAxises(points);
	console.log({ yAxises, pointsOfYAxises, data });
	...
	return (
		<ResponsiveContainer width='100%' height={'99%'}>
			<ComposedChart data={data}>
					...
					[...yAxises].map(([yAxisId, point], index) => {
						return isEnumPoint(point) ? (
							...
							<YAxis
								scale='linear'
								type='number'
								domain={getPointUnit(point) === '%' ? [0, 100] : ['auto', 'auto']}
								tick={{ stroke: '555', fontSize: 10, fontWeight: 400 }}
								stroke={getChartLineColor(point.uuid || '')}
								padding={{ bottom: 20, top: 20 }}
								yAxisId={yAxisId}
				 ...
				 *
				 */}
				<Legend verticalAlign='top' height={36} wrapperStyle={{ fontSize: '0.75em' }} content={<ChartLegend points={points} />} />
				...
				{points.filter(isEnumPoint).map(point => (
					<Line key={`$${point.uuid}`} dataKey={`$${point.uuid}`} yAxisId={pointsOfYAxises.get(point.uuid || '')} visibility='hidden' />
				))}
				{
					/**
